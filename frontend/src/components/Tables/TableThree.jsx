...
const Inventory = () => {
  ...
  const handleReorderClick = (productName) => {
    navigate('/profile', { state: { productName } });
  };
        ...
        <tbody>
          {inventory.map((item) => (
            <tr key={item._id} style={{ borderBottom: '1px solid #ddd' }}>
              ...
              <td>{item.name}</td>
              <td>{item.category}</td>
              <td style={{ color: item.in_stock <= item.reorder_level ? 'red' : 'green', fontWeight: 'bold' }}>
                {item.in_stock}
              </td>
              <td>{item.reorder_level}</td>
              <td>
                {item.in_stock <= item.reorder_level && (
                  <button onClick={() => handleReorderClick(item.name)}>Reorder</button>
                )}
              </td>
            </tr>
          ))}
